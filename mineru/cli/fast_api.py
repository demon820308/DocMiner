...
# Pydantic model for open_folder endpoint
class FolderOpenPayload(BaseModel):
    path: str
...
# Or use ModelScope: https://www.modelscope.cn
MODEL_SOURCE = os.getenv("MINERU_MODEL_SOURCE", "huggingface")
MODEL_CACHE_DIR = os.getenv("MINERU_MODEL_CACHE_DIR", "")

if MODEL_SOURCE == "modelscope":
    os.environ["HF_ENDPOINT"] = "https://www.modelscope.cn"
elif MODEL_SOURCE == "huggingface_mirror":
...
DEFAULT_OUTPUT_ROOT = "~/DocMiner/output"
...
class AsyncParseTask:
    ...
    def to_status_payload(
        ...
        payload = {
            ...
            "error": self.error,
            "output_dir": self.output_dir if self.status == TASK_COMPLETED else None,
            "status_url": str(
...
async def lifespan(app: FastAPI):
    ...
    # Start demo zip generation in background (non-blocking)
    asyncio.create_task(generate_demo_zip_if_missing())
    try:
        yield
    finally:
...
async def generate_demo_zip_if_missing() -> None:
    """Generate demo.zip from demo1.pdf if it doesn't exist."""
    global DEMO_ZIP_GENERATED
    async with DEMO_ZIP_LOCK:
        if DEMO_ZIP_GENERATED:
            return

        # Calculate paths
        project_root = Path(__file__).parent.parent.parent
        demo_pdf_path = project_root / "demo" / "pdfs" / "demo1.pdf"
        frontend_assets_dir = project_root / "projects" / "mineru-web" / "assets"
        demo_zip_path = frontend_assets_dir / "demo.zip"
        failed_marker = frontend_assets_dir / "demo.zip.failed"

        # Skip if already exists
        if demo_zip_path.exists():
            DEMO_ZIP_GENERATED = True
            return

        # Skip if failed marker exists (requires restart to retry)
        if failed_marker.exists():
            logger.warning("Demo zip generation previously failed. Restart service to retry.")
            return

        # Ensure demo PDF exists
        if not demo_pdf_path.exists():
            logger.warning(f"Demo PDF not found: {demo_pdf_path}")
            return
        ...
        try:
            ...
            logger.info(f"Generating demo zip from {demo_pdf_path}...")

            # Read PDF bytes
            pdf_bytes = demo_pdf_path.read_bytes()
            output_dir = str(frontend_assets_dir / "demo_output")

            # Call do_parse to parse the demo PDF
            do_parse(
                pdf_file_names=["demo1.pdf"],
            ...
            with zipfile.ZipFile(str(demo_zip_path), 'w', zipfile.ZIP_DEFLATED) as zf:
            ...
            DEMO_ZIP_GENERATED = True
            logger.info(f"Demo zip generated successfully: {demo_zip_path}")

        except Exception as e:
            # Create failed marker
            try:
                failed_marker.write_text(f"Failed at {utc_now_iso()}: {str(e)}")
            except Exception:
                pass
            logger.error(f"Failed to generate demo zip: {e}")
            # Don't block startup
...
class AsyncTaskManager:
    ...
    def get(self, task_id: str) -> Optional[AsyncParseTask]:
        ...
        # Try to reconstruct completed task from disk if server restarted
        try:
            import os
            task_output_dir = get_output_root() / task_id
            uploads_dir = task_output_dir / "uploads"
            if not task_output_dir.exists() or not uploads_dir.exists():
                return None
            ...
            file_stems = [os.path.splitext(f)[0] for f in file_names]
                
            backend = "vlm"
            parse_method = "auto"
            pdf_output_dir = task_output_dir / file_stems[0]
            if pdf_output_dir.exists() and pdf_output_dir.is_dir():
                for subdir in os.listdir(pdf_output_dir):
                    if subdir == "vlm":
                        backend = "vlm"
                        break
                    elif subdir.startswith("hybrid"):
                        backend = "hybrid"
                        if "_" in subdir:
                            parse_method = subdir.split("_", 1)[1]
                        break
            ...
            task = AsyncParseTask(
                ...
                start_page_id=0,
                end_page_id=0,
                upload_names=file_names,
                uploads=file_names,
            )
            # Cache the reconstructed task
            self.tasks[task_id] = task
            return task
        except Exception as e:
            logger.warning(f"Failed to reconstruct task {task_id} from disk: {e}")
            return None
...
async def delete_async_task(task_id: str):
    ...
    # Delete output directory if it exists, regardless of whether task is in memory
    output_root = get_output_root()
    task_output_dir = str(output_root / task_id)
    if os.path.exists(task_output_dir):
        try:
            shutil.rmtree(task_output_dir)
        except Exception as e:
            logger.error(f"Failed to delete output directory for task {task_id}: {e}")
            
    # Remove task from task manager if it is still in memory
    task_manager = get_task_manager()
    try:
        if task_id in task_manager.tasks:
            del task_manager.tasks[task_id]
        if task_id in task_manager.task_events:
            del task_manager.task_events[task_id]
    except Exception as e:
        logger.error(f"Failed to remove task {task_id} from task manager: {e}")
        
    return {"status": "success", "message": "Task and output directory deleted"}
...
@app.post(
    path="/api/open_folder",
    status_code=200,
    summary="Open folder in system file explorer",
    description="Open the specified folder path in the system's default file explorer.",
)
async def open_folder(payload: FolderOpenPayload, request: Request):
    client_ip = request.client.host if request.client else "unknown"
    folder_path = Path(payload.path).resolve()

    # Log the request
    logging.info(f"[open_folder] ip={client_ip} path={payload.path}")

    # Check if path exists
    if not folder_path.exists():
        logging.info(f"[open_folder] ip={client_ip} path={payload.path} status=not_found")
    ...
    try:
        ...
        logging.info(f"[open_folder] ip={client_ip} path={payload.path} status=success")
        return {"status": "success"}
    except Exception as e:
        logging.info(f"[open_folder] ip={client_ip} path={payload.path} status=error: {e}")
...
@app.post(
    ...
    status_code=200,
    summary="Update model download configuration",
    description="Configure model download source and cache directory.",
)
async def update_model_config(payload: ModelConfigPayload, request: Request):
    ...
    if payload.source == "modelscope":
        os.environ["HF_ENDPOINT"] = "https://www.modelscope.cn"
    elif payload.source == "huggingface_mirror":
        os.environ["HF_ENDPOINT"] = "https://hf-mirror.com"
    elif payload.source == "huggingface":
...
@click.option("--port", default=6060, type=int, help="Server port (default: 6060)")
